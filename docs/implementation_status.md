...
| 3. Rendering | **Complete (desktop-verified)** | axial/coronal/sagittal + 3D volume + MPR + CPR views in the main window layouts (MPR 3+1 / CPR 3+1), label overlays, scale bar, ROI markers, text annotations; zero-copy `VolumeData.image_data` |
| 4. Processing | **Complete (95 algorithms)** | full catalog: 16 filtering, 15 morphology, 8 thresholds (10 methods via native), 16 segmentation (incl. centerline + stent view), 6 reconstruction, 10 surface, 6 registration, 11 arithmetic, 5 enhancement, radiomics (114 features), measurement. **53 algorithms execute in C++/ITK** (native backend preferred; gaussian 64³ 2.4 ms native vs 9.9 s python binding). Texture radiomics (GLCM/GLRLM/GLSZM/GLDM/NGTDM) is C++ with a bit-consistent python fallback |
...
| 5b. Scripting | **Complete (core)** | medaxis facade, sandbox (3 levels), REPL console, script editor (QScintilla optional), action recorder, hook system, script manager with 5 templates, MCP run_script bridge — all verified E2E |
| 6. Geometry & advanced viz | **Complete** | native OCC curve sampling/Frenet; CGAL mesh ops; vessel centerline (Lee 3D thinning + Dijkstra + B-spline frames) and **stent view (circular unwrap with MIP)** verified; centerline loads into the CPR view from the View menu |
| 7. Plugins & ecosystem | **Complete** | discovery (.py + .pyd), manifests, lifecycle, model zoo + downloader; **plugin `medaxis_tools` auto-exposed to MCP** (verified over JSON-RPC) |
| 8. Persistence & workflow | **Complete (core)** | `.medaxis` archives, label panel + manual editor (8 tools + threshold paint) + undo/redo + label boolean ops, exports NIfTI/NRRD/DICOM/DICOM-SEG/**RTSTRUCT**/STL/OBJ/PLY/**3MF**/PNG/JPEG/PDF, comparison view (side-by-side/overlay/subtraction), 9 window/level presets, cine playback + **MP4 recording** (ffmpeg), autosave |
...
- DICOM SC/SEG/SR/RTSTRUCT writers are implemented; DICOMweb/gRPC AI clients
  and the nnU-Net/MedSAM/MONAI adapters remain stubs behind the REST client
  (external services are called through REST today).
- Natural-language parameter suggestion (Phase 5 P2) is not implemented;
  run_algorithm takes structured params directly.
...
- Debugger integration (debugpy) for scripts (Phase 5b P2) is not wired.
